...
class ContextKNN:
    ...
    def __init__( self, k = 500, sample_size=1000, sampling='recent',  similarity = 'cosine', remind=False, pop_boost=0, extend=False, normalize=True, session_key = 'SessionId', item_key= 'ItemId', time_key= 'Time' ):
    ...
    def fit_time_order_online(self, row, first_in_session, items=None):
        '''
        Trains the predictor.
        the dataset is order by time not ordered by sessions
        ...
        if first_in_session:
            session_items = set()
            session_items.add(row[self.item_key])
            self.session_item_map.update({current_session: session_items})
        else:
            session_items = self.session_item_map.get(current_session)
            session_items.add(row[self.item_key])
            self.session_item_map.update({current_session: session_items})

        self.session_time.update({current_session: row[self.time_key]})

        # cache sessions involving an item
        map_is = self.item_session_map.get(row[self.item_key])
        if map_is is None:
            map_is = set()
            self.item_session_map.update({row[self.item_key]: map_is})
        map_is.add(row[self.session_key])
...
#         print( 'cknn.predict_next: ', process.memory_info().rss, ' memory used')
        ...
        if self.normalize:
            predictions = preprocessing.normalize([predictions],'l2')
            #predictions = [i / sum(predictions) for i in predictions]
        series = pd.Series(data=predictions[0], index=predict_for_item_ids)
