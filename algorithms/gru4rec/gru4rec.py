...
class GRU4Rec:
    ...
    def fit(self, data, retrain=False, sample_store=10000000):
        ...
        itemids = data[self.item_key].unique() # unique item ids
        if not retrain:
    ...
    def predict_next(self, session_id, input_item_id, predict_for_item_ids, current_session_length, row, skip=False):
        '''
        Gives predicton scores for a selected set of items. Can be used in batch mode to predict for multiple independent events (i.e. events of different sessions) at once and thus speed up evaluation.
