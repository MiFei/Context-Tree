...
def evaluate_sessions(pr, metrics, test_data, train_data, isHybrid, isCT, items=None, cut_off=20, session_key='SessionId', item_key='ItemId', time_key='Time'):
    '''
    ...
    i = 0
    for t, row in test_data.iterrows():
...
#             preds += 1e-8 * np.random.rand(len(preds)) #Breaking up ties
        ...
        if isCT:
            trs = time.time();
            pr.match_context(row, items_to_predict, False)
            time_sum += time.time() - trs
    ...
    return res, time_sum, time_count


def evaluate_sessions_adapt(pr, metrics, test_data, train_data, prev_iid, rec_freshness, ground_truth_freshness, correct_rec_freshness, top_list, current_session_length, items=None, cut_off=20, session_key='SessionId',
    ...
    for t, row in test_data.iterrows():
        ...
        preds = pr.predict_next(sid, prev_iid[sid], items_to_predict)
        ...
        max_so_far = items_to_predict.max()
        rec_freshness = list(rec_freshness) + (np.array(preds[:5].index.tolist()) / max_so_far).tolist()
        ground_truth_freshness.append(iid/max_so_far)
        if iid in preds[:5]:
            correct_rec_freshness.append(iid/max_so_far)


        for m in metrics:
            m.add(preds, iid, top_list)
    ...
    return res , rec_freshness, ground_truth_freshness, correct_rec_freshness
